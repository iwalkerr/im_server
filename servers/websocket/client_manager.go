...
// 获取用户key
func GetUserKey(appId uint32, userId int) (key string) {
...
// 获取用户的连接
func (manager *ClientManager) GetUserClient(appId uint32, userId int) (client *Client) {
...
// 获取用户的key
func (manager *ClientManager) GetUserList(appId uint32) (userList []int) {
...
// 向全部成员(除了自己)发送数据
func (manager *ClientManager) sendAppIdAll(message []byte, teamId uint32, ignoreClient *Client) {

	clients := manager.GetUserClients()
	for _, conn := range clients {
		if conn != ignoreClient && conn.TeamId == teamId {
			conn.SendMsg(message)
		}
	}
}
...
func (manager *ClientManager) EventLogin(login *login) {
	...
	fmt.Println("EventLogin 用户登录", client.Addr, login.TeamId, login.UserId)

	// orderId := helper.GetOrderIdTime()
	SendUserMessageAll(login.TeamId, login.UserId, 1, models.MessageCmdEnter, "哈喽~")
}
...
func (manager *ClientManager) EventUnregister(client *Client) {
	...
	if client.UserId != 0 {
		// orderId := helper.GetOrderIdTime()
		SendUserMessageAll(client.TeamId, client.UserId, 1, models.MessageCmdExit, "用户已经离开~")
	}
}
...
// 获取用户所在的连接
func GetUserClient(appId uint32, userId int) (client *Client) {
...
// 获取全部用户
func GetUserList(teamId uint32) (userList []int) {
...
// 全员广播
func AllSendMessages(teamId uint32, userId int, data string) {
	// fmt.Println("全员广播", appId, userId, data)

	ignoreClient := clientManager.GetUserClient(teamId, userId)
	clientManager.sendAppIdAll([]byte(data), teamId, ignoreClient)
}
