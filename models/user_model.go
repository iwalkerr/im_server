...
// 用户登录
func UserLogin(accIp, accPort string, teamId, userId int, addr string, loginTime uint64) (userOnline *UserOnline) {
...
func (u *UserOnline) IsOnline() (online bool) {
	...
	if u.HeartbeatTime < (currentTime - heartbeatTimeout) {
		fmt.Println("用户是否在线 心跳超时", u.TeamId, u.UserId, u.HeartbeatTime)
		return
	}

	if u.IsLogoff {
		fmt.Println("用户是否在线 用户已经下线", u.TeamId, u.UserId)
		return
	}
	return true
}
